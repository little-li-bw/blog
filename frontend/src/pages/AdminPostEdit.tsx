...
import { getAdminPostDetail, updateAdminPost } from '../api/posts';
...
export default function AdminPostEdit() {
  ...
  React.useEffect(() => {
    ...
    Promise.all([getCategories(), getTags(), getAdminPostDetail(token, postId)])
      .then(([categoryList, tagList, post]) => {
        setCategories(categoryList);
        setTags(tagList);
        setTitle(post.title);
        setSummary(post.summary);
        setContentMd(post.contentMd ?? '');
        setCategoryId(String(post.categoryId ?? categoryList[0]?.id ?? ''));
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await updateAdminPost(token, postId, {
        title,
        summary,
        contentMd,
      ...
      <div className="max-w-5xl mx-auto px-6 py-10 space-y-8">
        <div className="flex items-end justify-between gap-6">
          <div className="space-y-2">
            ...
            <h1 className="text-4xl font-black tracking-tighter text-slate-900">编辑文章</h1>
          </div>
          <Link
            to="/admin/posts"
        ...
        <form className="bg-white border border-slate-200 shadow-sm p-8 space-y-6" onSubmit={handleSubmit}>
          {loading ? <div className="text-slate-500">正在加载文章配置...</div> : null}
